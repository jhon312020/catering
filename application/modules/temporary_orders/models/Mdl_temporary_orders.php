...
class Mdl_temporary_orders extends Response_Model {
    ...
    public function validation_rules() {
		...
		*/
		 ...
		 * 
		*/
		public function order_exists_update_or_insert($menuListIds, $purchasedItems, $purchaseIds) {
			...
			foreach($previousRecords as $prevRecord) {
				...
				//echo $order_type;die;
				if(!in_array($prevRecord['menu_id'], $newMenuIds) && $order_type != $prevRecord['order_type']) {
					
					//echo $prevRecord['id'];die;
					$this->mdl_temporary_orders->save($prevRecord['id'], array('order_type' => $order_type));
				}
			}
			$data = [];
			foreach($newMenuIds as $menu_id) {
				...
				$data[] = array('menu_id' => $menu_id, 'order_date' => date('Y-m-d'), 'client_id' => $this->session->userdata('client_id'), 'order_type' => $order_type);
			}
			//print_r($data);die;
			if($newMenuIds) {
